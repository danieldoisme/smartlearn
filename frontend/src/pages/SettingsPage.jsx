...
const displayModes = [
  {
    value: DisplayMode.IMMEDIATE,
    label: DisplayModeLabel[DisplayMode.IMMEDIATE],
    desc: "Xem đáp án ngay sau mỗi câu",
  },
  {
    value: DisplayMode.END,
    label: DisplayModeLabel[DisplayMode.END],
    desc: "Xem tất cả đáp án sau khi hoàn thành",
  },
];
...
function SettingsContent({ me, prefs }) {
  ...
  useEffect(() => {
    if (
      preferences.defaultQuestionCount === prefs?.defaultQuestionCount &&
      preferences.preferredQuestionType === prefs?.preferredQuestionType &&
      preferences.answerDisplayMode === prefs?.answerDisplayMode
    )
      return;
    ...
    handlePreferencesSave,
    preferences,
    prefs?.answerDisplayMode,
    prefs?.defaultQuestionCount,
    prefs?.preferredQuestionType,
  ]);
  ...
  const handleAvatarPick = (e) => {
    ...
    if (!ACCEPTED_AVATAR_TYPES.includes(file.type)) {
      setAvatarError(
        "Định dạng ảnh không hỗ trợ. Dùng JPG, PNG, WEBP hoặc GIF.",
      );
      return;
    }
                ...
                <div className="flex items-center gap-3">
                  ...
                  {updateMe.error && (
                    <span className="text-xs text-red-600">
                      {updateMe.error?.response?.data?.detail ||
                        "Lỗi cập nhật."}
                    </span>
                  )}
                </div>
              ...
              <CardContent className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-slate-800 mb-1">
                    Số câu hỏi mặc định mỗi phiên
                  </label>
                  <p className="text-xs text-slate-500 mb-3">
                    Áp dụng khi bắt đầu phiên học tập mới
                  </p>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mb-3">
                    Loại câu hỏi được ưu tiên khi tạo tự động
                  </p>
                  <div className="flex gap-2">
                    {questionTypes.map((qt) => (
                      <button
                        ...
                        onClick={() =>
                          setPreferences({
                            ...preferences,
                            preferredQuestionType: qt.value,
                          })
                        }
                        className={`px-4 py-2 text-sm font-medium rounded-xl transition-all cursor-pointer ${
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mb-3">
                    Cách hiển thị kết quả trong phiên học tập
                  </p>
                ...
                <div className="flex items-center gap-3 h-10">
                  {updatePreferences.isPending ? (
                    ...
                    <span className="text-xs text-red-600">
                      {updatePreferences.error?.response?.data?.detail ||
                        "Lỗi cập nhật."}
                    </span>
                  ) : (
